@if ($value->status == 1)
    ...
    <span class="badge rounded-pill bg-success @if (empty($isused)) changestatus  pointer @endif"
        @if (empty($isused)) onclick="changeStatus('{{ $value->uuid }}', {{ config('params.in_active') }},$(this))" @endif
        data-url="{{ route('plans.changestatus') }}" data-status="InActive"
...
@elseif ($value->status == 2)
    ...
    <span class="badge rounded-pill bg-warning changestatus pointer"
        ...
        data-url="{{ route('plans.changestatus') }}" data-status="Active" title="click to Active">Inactive</span>
