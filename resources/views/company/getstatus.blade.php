@if ($value->status == 1)
    ...
    <span class="badge rounded-pill bg-success changestatus pointer"
        onclick="changeStatus('{{ $value->uuid }}', {{ config('params.in_active') }},$(this))"
        data-url="{{ route('company.changestatus') }}" data-status="InActive" title="click to inactive"
...
@elseif ($value->status == 2)
    ...
    <span class="badge rounded-pill bg-warning changestatus pointer"
        ...
        data-url="{{ route('company.changestatus') }}" data-status="Active" title="click to Active">Inactive</span>
